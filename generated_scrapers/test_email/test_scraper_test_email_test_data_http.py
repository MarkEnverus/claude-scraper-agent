"""Tests for TestEMAIL test_data collector.
...
from test_email.scraper_test_email_test_data_http import (
    TestemailTestDataCollector,
    ENDPOINTS,
)
...
def collector(mock_redis, mock_s3_manager, mock_kafka_producer):
    """Create TestEMAIL collector instance with mocked dependencies."""
    return TestemailTestDataCollector(
        api_key="TEST_EMAIL_API_KEY_TEST_KEY",
...
def sample_candidate():
    ...
    return DownloadCandidate(
        ...
        expected_filename="test_email_test_data_test_20250101.json",
        metadata={
            "source": "testemail",
            "data_type": "test_data",
            "endpoint": "test",
...
def test_collector_initialization(collector):
    ...
    assert collector.dgroup == "test_email_test_data"
...
def test_generate_candidates_single_day(collector):
    ...
    assert candidate.metadata["source"] == "testemail"
...
def test_generate_filename(collector):
    ...
    assert filename.startswith("test_email_test_data")
...
def test_cli_required_parameters():
    ...
    from test_email.scraper_test_email_test_data_http import main
...
def test_cli_environment_variables():
    ...
    with runner.isolated_filesystem():
        ...
        assert "TEST_EMAIL_API_KEY" not in result.output or result.exit_code == 0
...
@pytest.mark.skipif(
    os.getenv("TEST_EMAIL_API_KEY") is None,
    reason="Requires TEST_EMAIL_API_KEY environment variable",
)
...
def test_integration_real_api_call():
    ...
    Set TEST_EMAIL_API_KEY environment variable to run.
    """
    import redis
    ...
    collector = TestemailTestDataCollector(
        ...
        dgroup="test_email_test_data_test",
    )
