...
This scraper collects test_data data from TestAPI.
...
class TestapiTestDataCollector(BaseCollector):
    """Collector for TestAPI test_data data.
    ...
    def __init__(
        ...
        dgroup: str = "test_api_test_data",
    ):
        """Initialize TestAPI collector.
    ...
    def _generate_filename(self, endpoint_name: str, target_date: date) -> str:
        ...
        return f"test_api_test_data_{endpoint_name}_{date_str}.json"
...
@click.option(
    "--api-key",
    envvar="TEST_API_API_KEY",
    help="NONE authentication key (can use TEST_API_API_KEY env var)",
)
@click.option(
    "--redis-host",
    default="localhost",
...
def main(
    ...
    Collects test_data data from TestAPI for the specified date range.
    ...
    logger.info(
        "Starting TestAPI test_data collector",
        extra={
    ...
    try:
        ...
        logger.error(
            f"Fatal error in TestAPI collector: {e}",
            exc_info=True,
        )
        sys.exit(1)
