"""Tests for TestFTP test_data collector.
...
from test_ftp.scraper_test_ftp_test_data_http import (
    TestftpTestDataCollector,
    ENDPOINTS,
)
...
def collector(mock_redis, mock_s3_manager, mock_kafka_producer):
    """Create TestFTP collector instance with mocked dependencies."""
    return TestftpTestDataCollector(
        api_key="TEST_FTP_API_KEY_TEST_KEY",
        redis_client=mock_redis,
        s3_manager=mock_s3_manager,
        kafka_producer=mock_kafka_producer,
        dgroup="test_ftp_test_data",
    )
...
def sample_candidate():
    ...
    return DownloadCandidate(
        ...
        expected_filename="test_ftp_test_data_test_20250101.json",
        metadata={
            "source": "testftp",
            "data_type": "test_data",
            "endpoint": "test",
...
def test_collector_initialization(collector):
    ...
    assert collector.dgroup == "test_ftp_test_data"
...
def test_collector_missing_api_key():
    ...
    collector = TestftpTestDataCollector(
        api_key=None,
        redis_client=Mock(),
        s3_manager=Mock(),
        kafka_producer=Mock(),
...
def test_generate_candidates_single_day(collector):
    ...
    assert candidate.metadata["source"] == "testftp"
...
def test_generate_filename(collector):
    ...
    assert filename.startswith("test_ftp_test_data")
...
def test_cli_required_parameters():
    ...
    from test_ftp.scraper_test_ftp_test_data_http import main
...
def test_cli_environment_variables():
    ...
    with runner.isolated_filesystem():
        ...
        assert "TEST_FTP_API_KEY" not in result.output or result.exit_code == 0
...
@pytest.mark.skipif(
    os.getenv("TEST_FTP_API_KEY") is None,
    reason="Requires TEST_FTP_API_KEY environment variable",
)
...
def test_integration_real_api_call():
    ...
    Set TEST_FTP_API_KEY environment variable to run.
    """
    import redis
    ...
    collector = TestftpTestDataCollector(
        api_key=os.getenv("TEST_FTP_API_KEY"),
        redis_client=redis_client,
        s3_manager=s3_manager,
        kafka_producer=kafka_producer,
        dgroup="test_ftp_test_data_test",
    )
