# TestFTP Test_data Data Collector

Automated data collector for TestFTP test_data data.

## Overview

This scraper collects test_data data from TestFTP using their FTP_SFTP interface.
...
| `KAFKA_TOPIC` | No | Kafka topic (default: test_ftp_test_data) |
...
```bash
python scraper_test_ftp_test_data_http.py \
...
| `--dgroup` | Data group identifier (default: test_ftp_test_data) |
...
```bash
export TEST_FTP_API_KEY="your-api-key"
...
└── test_ftp_test_data/
    └── YYYY/
        └── MM/
            └── DD/
                └── test_ftp_test_data_ENDPOINT_YYYYMMDD_HHMMSS_VERSION.json
```

Example:
```
s3://production-data/test_ftp_test_data/2025/01/15/test_ftp_test_data_test_20250115_143022_001.json
```
...
```json
{
    ...
    "endpoint": "test",
    "date": "2025-01-15",
    "s3_path": "s3://bucket/test_ftp_test_data/2025/01/15/file.json",
    "file_hash": "abc123...",
...
```bash
pytest tests/test_ftp/test_scraper_test_ftp_test_data_http.py -v
```
...
```bash
export TEST_FTP_API_KEY="your-api-key"
pytest tests/test_ftp/test_scraper_test_ftp_test_data_http.py -v -m integration
```

### Test Coverage

```bash
pytest tests/test_ftp/ --cov=test_ftp --cov-report=html
```
...
- Verify TEST_FTP_API_KEY is set correctly
...
python -m claude_scraper.updater \
    --scraper scraper_test_ftp_test_data_http.py \
