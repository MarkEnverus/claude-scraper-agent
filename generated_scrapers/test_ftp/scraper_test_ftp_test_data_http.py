...
This scraper collects test_data data from TestFTP.
...
Collection Method: FTP_SFTP
Scraper Type: ftp-sftp
...
class TestftpTestDataCollector(BaseCollector):
    """Collector for TestFTP test_data data.
    ...
    def __init__(
        ...
        dgroup: str = "test_ftp_test_data",
    ):
        """Initialize TestFTP collector.
    ...
    def _generate_filename(self, endpoint_name: str, target_date: date) -> str:
        ...
        return f"test_ftp_test_data_{endpoint_name}_{date_str}.json"
...
@click.option(
    "--api-key",
    envvar="TEST_FTP_API_KEY",
    help="NONE authentication key (can use TEST_FTP_API_KEY env var)",
)
@click.option(
    "--redis-host",
    default="localhost",
...
def main(
    ...
    Collects test_data data from TestFTP for the specified date range.
    ...
    logger.info(
        "Starting TestFTP test_data collector",
        extra={
    ...
    try:
        ...
        logger.error(
            f"Fatal error in TestFTP collector: {e}",
            exc_info=True,
        )
        sys.exit(1)
