...
This scraper collects test_data data from TestWEBSITE.
...
Collection Method: WEBSITE_PARSER
Scraper Type: website-parser
...
class TestwebsiteTestDataCollector(BaseCollector):
    """Collector for TestWEBSITE test_data data.
    ...
    def __init__(
        ...
        dgroup: str = "test_website_test_data",
    ):
        """Initialize TestWEBSITE collector.
    ...
    def _init_auth(self) -> None:
        ...
        logger.info(
            "Initialized TestWEBSITE collector",
            extra={
    ...
    def _generate_filename(self, endpoint_name: str, target_date: date) -> str:
        ...
        return f"test_website_test_data_{endpoint_name}_{date_str}.json"
...
@click.option(
    "--api-key",
    envvar="TEST_WEBSITE_API_KEY",
    help="NONE authentication key (can use TEST_WEBSITE_API_KEY env var)",
)
@click.option(
    "--redis-host",
    default="localhost",
...
@click.option(
    "--kafka-topic",
    default="test_website_test_data",
    envvar="KAFKA_TOPIC",
...
def main(
    ...
    Collects test_data data from TestWEBSITE for the specified date range.
    ...
    logger.info(
        "Starting TestWEBSITE test_data collector",
        extra={
    ...
    try:
        ...
        logger.error(
            f"Fatal error in TestWEBSITE collector: {e}",
            exc_info=True,
        )
        sys.exit(1)
