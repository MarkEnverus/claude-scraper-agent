"""Tests for TestWEBSITE test_data collector.
...
from test_website.scraper_test_website_test_data_http import (
    TestwebsiteTestDataCollector,
    ENDPOINTS,
)
...
def collector(mock_redis, mock_s3_manager, mock_kafka_producer):
    """Create TestWEBSITE collector instance with mocked dependencies."""
    return TestwebsiteTestDataCollector(
        api_key="TEST_WEBSITE_API_KEY_TEST_KEY",
...
def sample_candidate():
    ...
    return DownloadCandidate(
        ...
        expected_filename="test_website_test_data_test_20250101.json",
        metadata={
            "source": "testwebsite",
            "data_type": "test_data",
            "endpoint": "test",
...
def test_collector_initialization(collector):
    ...
    assert collector.base_url == "https://test.example.com"
    assert collector.dgroup == "test_website_test_data"
...
def test_collector_missing_api_key():
    ...
    collector = TestwebsiteTestDataCollector(
        api_key=None,
        redis_client=Mock(),
        s3_manager=Mock(),
        kafka_producer=Mock(),
...
def test_endpoint_configuration():
    ...
    assert endpoint_0["name"] == "test"
    assert endpoint_0["method"] == "GET"
    assert endpoint_0["path"] == "/test"
...
def test_generate_candidates_single_day(collector):
    ...
    assert candidate.metadata["source"] == "testwebsite"
    assert candidate.metadata["data_type"] == "test_data"
...
def test_build_url_with_date(collector):
    ...
    assert url.startswith("https://test.example.com")
...
def test_generate_filename(collector):
    ...
    assert filename.startswith("test_website_test_data")
...
def test_cli_required_parameters():
    ...
    from test_website.scraper_test_website_test_data_http import main
...
def test_cli_environment_variables():
    ...
    with runner.isolated_filesystem():
        ...
        assert "TEST_WEBSITE_API_KEY" not in result.output or result.exit_code == 0
...
@pytest.mark.skipif(
    os.getenv("TEST_WEBSITE_API_KEY") is None,
    reason="Requires TEST_WEBSITE_API_KEY environment variable",
)
...
def test_integration_real_api_call():
    ...
    Set TEST_WEBSITE_API_KEY environment variable to run.
    """
    import redis
    ...
    collector = TestwebsiteTestDataCollector(
        api_key=os.getenv("TEST_WEBSITE_API_KEY"),
        redis_client=redis_client,
        s3_manager=s3_manager,
        kafka_producer=kafka_producer,
        dgroup="test_website_test_data_test",
    )
