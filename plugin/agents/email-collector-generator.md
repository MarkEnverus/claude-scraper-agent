...
```python
"""
{Source} {Type} Email Scraper

Downloads attachments from emails for {source} {type} data.

Author: Generated by Claude Scraper Agent
Date: {current_date}
"""
...
def main(
    ...
    email_host = os.getenv("{SOURCE_UPPER}_EMAIL_HOST")
    email_port = int(os.getenv("{SOURCE_UPPER}_EMAIL_PORT", "993"))
    email_username = os.getenv("{SOURCE_UPPER}_EMAIL_USERNAME")
    email_password = os.getenv("{SOURCE_UPPER}_EMAIL_PASSWORD")
    mailbox = os.getenv("{SOURCE_UPPER}_EMAIL_MAILBOX", "INBOX")
    subject_filter = os.getenv("{SOURCE_UPPER}_SUBJECT_FILTER")
    sender_filter = os.getenv("{SOURCE_UPPER}_SENDER_FILTER")
    attachment_pattern = os.getenv("{SOURCE_UPPER}_ATTACHMENT_PATTERN", r".*\.csv")

    if not all([email_host, email_username, email_password]):
        raise ValueError("Missing required environment variables: {SOURCE_UPPER}_EMAIL_HOST, {SOURCE_UPPER}_EMAIL_USERNAME, {SOURCE_UPPER}_EMAIL_PASSWORD")
