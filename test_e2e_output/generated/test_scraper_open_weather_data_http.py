"""Tests for OpenWeather data collector.
...
from scraper_open_weather_data_http import (
    OpenweatherDataCollector,
    ENDPOINTS,
)
...
def collector(mock_redis, mock_s3_manager, mock_kafka_producer):
    """Create OpenWeather collector instance with mocked dependencies."""
    return OpenweatherDataCollector(
        api_key="OPEN_WEATHER_API_KEY_TEST_KEY",
...
def sample_candidate():
    ...
    return DownloadCandidate(
        url="/weather",
        expected_filename="open_weather_data_test_20250101.json",
        metadata={
            "source": "openweather",
            "data_type": "data",
...
def test_collector_initialization(collector):
    ...
    assert collector.dgroup == "open_weather_data"
...
def test_endpoint_configuration():
    ...
    assert endpoint_0["name"] == "current_weather"
    assert endpoint_0["method"] == "GET"
    assert endpoint_0["path"] == "/weather"
    # Get 5-day weather forecast
    endpoint_1 = ENDPOINTS[1]
    assert endpoint_1["name"] == "forecast"
    assert endpoint_1["method"] == "GET"
    assert endpoint_1["path"] == "/forecast"
...
def test_generate_candidates_single_day(collector):
    ...
    assert candidate.metadata["source"] == "openweather"
    assert candidate.metadata["data_type"] == "data"
...
def test_generate_filename(collector):
    ...
    assert filename.startswith("open_weather_data")
...
def test_cli_required_parameters():
    ...
    from open_weather.scraper_open_weather_data_http import main
...
def test_cli_environment_variables():
    ...
    with runner.isolated_filesystem():
        ...
        assert "OPEN_WEATHER_API_KEY" not in result.output or result.exit_code == 0
...
@pytest.mark.skipif(
    os.getenv("OPEN_WEATHER_API_KEY") is None,
    reason="Requires OPEN_WEATHER_API_KEY environment variable",
)
...
def test_integration_real_api_call():
    ...
    Set OPEN_WEATHER_API_KEY environment variable to run.
    """
    import redis
