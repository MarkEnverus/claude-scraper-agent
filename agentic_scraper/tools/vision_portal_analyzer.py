...
class VisionPortalAnalyzer:
    ...
    def analyze_portal(
        ...
        """Analyze portal using vision to understand its structure across ALL available screenshots.

        Args:
            url: URL of the portal
            screenshot_paths: Path(s) to screenshot(s) - accepts both single string and list
        ...
        """
        # FIX 3: Handle both single screenshot and list for backwards compatibility
        if isinstance(screenshot_paths, str):
            screenshot_paths = [screenshot_paths]

        logger.info("=== VISION-BASED PORTAL ANALYSIS ===")
        logger.info(f"Analyzing: {url}")
        logger.info(f"Using {len(screenshot_paths)} screenshot(s)")
...
Be specific and concrete. Focus on what you actually see, not assumptions."""
        ...
        try:
            ...
            from agentic_scraper.types import VisionPortalAnalysisResult

            # FIX 3: Pass ALL screenshots to vision API
            result = self.llm.invoke_structured_with_vision(
                prompt=prompt,
                images=screenshot_paths,  # All screenshots, not just first
                response_model=VisionPortalAnalysisResult,
                system="You are an expert at analyzing API documentation portals visually. Look across ALL screenshots to understand the complete portal structure."
            )
