...
# Mock confluent_kafka before any imports that might use it
# This is needed because confluent-kafka is not installed in test environment
import sys
...
class TestKafkaConfiguration:
    ...
    def test_simple_connection_string(self):
        ...
        conn_str = "kafka://kafka.test.com:9092/my-topic"
        config = KafkaConfiguration(conn_str)

        assert config.bootstrap_server == "kafka.test.com:9092"
    ...
    def test_connection_string_with_sasl_plaintext(self):
        ...
        conn_str = "kafka://kafka.test.com:9092/my-topic?security_protocol=SASL_PLAINTEXT&sasl_mechanism=PLAIN"
        config = KafkaConfiguration(conn_str)
    ...
    def test_producer_cfg_dict_plaintext(self):
        ...
        assert cfg_dict["bootstrap.servers"] == "kafka.test.com:9092"
    ...
    def test_producer_cfg_dict_with_sasl(self):
        ...
        conn_str = "kafka://kafka.test.com:9092/my-topic?security_protocol=SASL_PLAINTEXT"
    ...
    def test_read_sasl_credentials_from_file(self):
        ...
        with patch("builtins.open", mock_open(read_data=creds_json)), \
             patch.object(KafkaConfiguration, "_validate_sasl_file_path", return_value=None):
            conn_str = "kafka://kafka.test.com:9092/my-topic?security_protocol=SASL_PLAINTEXT&X_sasl_file=/path/to/creds.json"
            config = KafkaConfiguration(conn_str)
...
class TestKafkaProducer:
    ...
    @patch("commons.kafka_utils.KafkaProducer.__init__", return_value=None)
